...
Proofs mined by a trustless instance are based on the BLAKE3 regular hash function, and are verifiable by any instance that shares the same context and configuration. Proofs mined by a permissioned instance are based on the BLAKE3 keyed hash function, and are verifiable only by instances that share the same context, configuration, and shared secret.
